...
import {CLEAR_SHOPCART} from '../../store/mutations_types'
export default {
  components: {
    CartControl
  },
  ...
  computed: {
    ...mapGetters(['totalCount', 'totalPrice']), //getters只能使用数组的形式
    ...mapState({
      info: state => state.shop.shopData.info,
      cartShop: state => state.shop.cartShop
    }),
    payContent(){
  ...
  methods: {
    ...
    clearShopCart(){
      //清空数据，但是不能直接再这里处理，需要在mutations中处理
      this.$store.commit(CLEAR_SHOPCART);
    }
  },
  watch: {
    //监视cartShop，一旦cartShop中没有数据，那么购物车列表也将会隐藏
    cartShop(newValue){
